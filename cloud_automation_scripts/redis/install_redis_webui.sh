...
installAndConfigureRedisWebui
startRedis $REDIS_PORT_WEBUI
...
function checkForRedisMasterToBeOnMasterIp(){
	PONG=$( (echo 'ping'; sleep 2) | telnet $REDIS_MASTER_IP_WEBUI $REDIS_PORT_WEBUI | grep '+PONG')
	ROLE_MASTER=$( ( echo 'info replication'; sleep 2) | telnet $REDIS_MASTER_IP_WEBUI $REDIS_PORT_WEBUI | grep 'role:master')
	sleep 5
}

function monitorAndHandleVIP(){
	logMessageToFile "INFO"  "Starting to monitor VIP on master ip: $REDIS_MASTER_IP_WEBUI"
	i=1
	while [ $i -le 10 ]; do
	...
	if [ -z "$ROLE_MASTER" ]; then
		...
		MASTER_ID=$(nova list | grep $REDIS_MASTER_IP_WEBUI | awk -F'|' '{ print $2}' | tr -d ' ')
		MASTER_NAME=$(nova list | grep $MASTER_ID | awk -F'|' '{ print $3}' | tr -d ' ')
		logMessageToFile "INFO"  "Old MASTER_ID is: $MASTER_ID"
		MASTER_INTERFACE=$(nova interface-list $MASTER_ID | grep $REDIS_MASTER_IP_WEBUI | awk -F'|' '{ print $3}' | tr -d ' ')
		...
		ATTACH_TO=$(nova list | grep $REDIS_SERVER_GENERIC_WEBUI | grep -v $MASTER_ID | awk -F'|' ' { print $3}' )
		logMessageToFile "INFO"  "Attaching master ip: $REDIS_MASTER_IP_WEBUI to instance $ATTACH_TO ... "
		nova interface-attach $ATTACH_TO --net-id $REDIS_NET_ID  --fixed-ip $REDIS_MASTER_IP_WEBUI
		PORT_ID=$(nova interface-list $ATTACH_TO | grep $REDIS_MASTER_IP_WEBUI | awk -F'|' '{print $3}' | tr -d ' ')
...
REDIS_SECOND_INSTANCE=$(nova list | grep $REDIS_SERVER_GENERIC_WEBUI | grep -v $REDIS_MASTER_IP_WEBUI | grep -i active  | grep -i RUNNING | awk -F'|' '{print $7}' | awk -F'=' '{print $2}' | awk -F',' '{ print $1 }' | tr -d ' ')
sleep 10
done
logMessageToFile "INFO"  "configuring sentinel rescue on: $REDIS_SECOND_INSTANCE"
echo 'daemonize yes
logfile /var/log/sentinel_2'$REDIS_PORT_WEBUI'.log

sentinel monitor mymaster '$REDIS_MASTER_IP_WEBUI' '$REDIS_PORT_WEBUI ' 1
...
function main(){
	...
	MASTER_ME=$( ( echo 'info replication'; sleep 1) | telnet localhost $REDIS_PORT_WEBUI | grep 'role:master')
	if [ -z "$MASTER_ME" ]; then
		monitorAndHandleVIP
	fi
